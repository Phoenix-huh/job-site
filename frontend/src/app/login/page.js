          ...
          <div className="relative z-20 flex items-end justify-center h-full mt-10">
            <div className="relative w-full h-[340px] flex items-end justify-center overflow-visible">
              <div ref={purpleRef} className="absolute bottom-0 transition-all duration-700 ease-in-out" style={{
                left: '10%', width: '35%', height: (isTyping || (password.length > 0 && !showPassword)) ? '340px' : '300px',
                backgroundColor: '#7A40F2', borderRadius: '16px 16px 0 0', zIndex: 1,
                transform: (password.length > 0 && showPassword) ? `skewX(0deg)` : (isTyping || (password.length > 0 && !showPassword)) ? `skewX(${(purplePos.bodySkew || 0) - 12}deg) translateX(20px)` : `skewX(${purplePos.bodySkew || 0}deg)`,
                ...
                <div className="absolute flex gap-6 transition-all duration-700 ease-in-out" style={{
                  ...
                  <EyeBall size={16} pupilSize={6} maxDistance={4} isBlinking={isPurpleBlinking} forceLookX={(password.length > 0 && showPassword) ? (isPurplePeeking ? 3 : -3) : isLookingAtEachOther ? 2 : undefined} forceLookY={(password.length > 0 && showPassword) ? (isPurplePeeking ? 4 : -3) : isLookingAtEachOther ? 3 : undefined} />
                  <EyeBall size={16} pupilSize={6} maxDistance={4} isBlinking={isPurpleBlinking} forceLookX={(password.length > 0 && showPassword) ? (isPurplePeeking ? 3 : -3) : isLookingAtEachOther ? 2 : undefined} forceLookY={(password.length > 0 && showPassword) ? (isPurplePeeking ? 4 : -3) : isLookingAtEachOther ? 3 : undefined} />
                </div>
              </div>
              <div ref={blackRef} className="absolute bottom-0 transition-all duration-700 ease-in-out" style={{
                left: '42%', width: '24%', height: '240px', backgroundColor: '#2A2A2B', borderRadius: '12px 12px 0 0', zIndex: 2,
                transform: (password.length > 0 && showPassword) ? `skewX(0deg)` : isLookingAtEachOther ? `skewX(${(blackPos.bodySkew || 0) * 1.5 + 8}deg) translateX(10px)` : (isTyping || (password.length > 0 && !showPassword)) ? `skewX(${(blackPos.bodySkew || 0) * 1.5}deg)` : `skewX(${blackPos.bodySkew || 0}deg)`,
                transformOrigin: 'bottom center',
              }}>
                <div className="absolute flex gap-4 transition-all duration-700 ease-in-out" style={{
                  ...
                  <EyeBall size={14} pupilSize={5} maxDistance={3} isBlinking={isBlackBlinking} forceLookX={(password.length > 0 && showPassword) ? -3 : isLookingAtEachOther ? 0 : undefined} forceLookY={(password.length > 0 && showPassword) ? -3 : isLookingAtEachOther ? -3 : undefined} />
                  <EyeBall size={14} pupilSize={5} maxDistance={3} isBlinking={isBlackBlinking} forceLookX={(password.length > 0 && showPassword) ? -3 : isLookingAtEachOther ? 0 : undefined} forceLookY={(password.length > 0 && showPassword) ? -3 : isLookingAtEachOther ? -3 : undefined} />
                </div>
              </div>
              <div ref={orangeRef} className="absolute bottom-0 transition-all duration-700 ease-in-out" style={{
                left: '0%', width: '50%', height: '160px', zIndex: 3, backgroundColor: '#FD8A5E', borderRadius: '160px 160px 0 0',
                transform: (password.length > 0 && showPassword) ? `skewX(0deg)` : `skewX(${orangePos.bodySkew || 0}deg)`, transformOrigin: 'bottom center',
              }}>
                <div className="absolute flex gap-[22px] transition-all duration-200 ease-out" style={{
              ...
              <div ref={yellowRef} className="absolute bottom-0 transition-all duration-700 ease-in-out" style={{
                left: '58%', width: '30%', height: '180px', backgroundColor: '#E7CE4F', borderRadius: '80px 80px 0 0', zIndex: 4,
                transform: (password.length > 0 && showPassword) ? `skewX(0deg)` : `skewX(${yellowPos.bodySkew || 0}deg)`, transformOrigin: 'bottom center',
              }}>
                <div className="absolute flex gap-[18px] transition-all duration-200 ease-out" style={{
                ...
                <div className="absolute h-[3px] bg-[#2D2D2D] rounded-full transition-all duration-200 ease-out" style={{
                  width: '45%', left: (password.length > 0 && showPassword) ? `10%` : `calc(50% - 22% + ${yellowPos.faceX || 0}px)`,
          ...
          <div className="w-full max-w-[360px]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input id="email" type="email" placeholder="anna@gmail.com" value={email} autoComplete="off" onChange={(e) => setEmail(e.target.value)} onFocus={() => setIsTyping(true)} onBlur={() => setIsTyping(false)} required className="h-11 bg-white border-neutral-200 text-neutral-900 placeholder:text-neutral-400 focus:border-zinc-800 focus:ring-zinc-800 rounded-lg transition-colors" />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="password" className="text-[13px] font-semibold text-neutral-800">Password</Label>
                <div className="relative">
                  <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required className="h-11 pr-10 bg-white border-neutral-200 text-neutral-900 placeholder:text-neutral-400 focus:border-zinc-800 focus:ring-zinc-800 rounded-lg transition-colors" />
              ...
              <div className="flex items-center justify-between pt-1">
                <div className="flex items-center space-x-2">
                  <Checkbox id="remember" className="border-neutral-300 data-[state=checked]:bg-zinc-900 data-[state=checked]:border-zinc-900 size-4 rounded-[4px]" />
                  <Label htmlFor="remember" className="text-[13px] font-medium text-neutral-700 cursor-pointer">Remember for 30 days</Label>
                </div>
                <a href="#" className="text-[13px] text-zinc-900 hover:underline font-semibold">Forgot password?</a>
              </div>
              {error && (
                <div className="p-3 text-[13px] text-red-600 bg-red-50 border border-red-100 rounded-lg font-medium">{error}</div>
              )}
              <Button type="submit" className="w-full h-11 text-[14px] font-semibold bg-zinc-900 hover:bg-zinc-800 text-white rounded-lg mt-2 transition-colors" disabled={isLoading}>
                {isLoading ? "Signing in..." : "Log in"}
              </Button>
            </form>
            <div className="text-center text-[13px] text-neutral-500 mt-6">
              Don&apos;t have an account?{" "}
              <a href="/register" className="text-zinc-900 font-bold hover:underline">Sign Up</a>
            </div>
          </div>
        </div>
      </div>
